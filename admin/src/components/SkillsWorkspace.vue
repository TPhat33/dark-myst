...
import { ApiError, getCurrent, listVersions, newIdempotencyKey, publishSkills, rollbackTo, validateSkills } from '../api'
import { diffList } from '../lib/diff'
import type { AdminValidateResult, AdminVersionEntry, SkillDefinition } from '../types'
...
const originalSkills = ref<SkillDefinition[]>([])
const editable = reactive<{ skills: SkillDefinition[] }>({ skills: [] })

const selectedId = ref<string | null>(null)
const selected = computed(() => editable.skills.find((s) => s.id === selectedId.value) ?? null)
...
const pendingDiff = computed(() => diffList(originalSkills.value, editable.skills))
const hasPendingChanges = computed(
  () => pendingDiff.value.changed.length > 0 || pendingDiff.value.added.length > 0 || pendingDiff.value.removed.length > 0
)

function cloneAll(skills: SkillDefinition[]): SkillDefinition[] {
  return skills.map((s) => JSON.parse(JSON.stringify(s)) as SkillDefinition)
}
...
async function loadCurrent() {
  ...
  if (result) {
    ...
    originalSkills.value = cloneAll(result.skills)
    editable.skills = cloneAll(result.skills)
    if (!selectedId.value && editable.skills.length > 0) {
      selectedId.value = editable.skills[0].id
    }
...
function resetLocalEdits() {
  editable.skills = cloneAll(originalSkills.value)
  validation.value = null
}
...
async function runValidate() {
  ...
  validation.value = null
  try {
    const result = await handleUnauthorized(() => validateSkills(props.adminToken, editable.skills))
    if (result) {
...
async function runPublish() {
  ...
  try {
    const result = await handleUnauthorized(() =>
      publishSkills(props.adminToken, editable.skills, publishNotes.value, newIdempotencyKey())
    )
    if (result) {
...
<template>
  ...
  <div v-else class="workspace">
    <p v-if="loadError" class="error" data-testid="skill-load-error">{{ loadError }}</p>

    <div class="version-banner card">
      <strong>Published version:</strong> <span data-testid="skill-current-version">{{ currentVersion }}</span>
      <span v-if="hasPendingChanges" class="pending-badge" data-testid="skill-pending-badge">unsaved edits</span>
    </div>
    ...
    <div class="columns">
      <section class="card item-list">
        <h2>Skills</h2>
        <ul>
          <li v-for="s in editable.skills" :key="s.id">
            <button
              class="item-button"
              :class="{ active: s.id === selectedId }"
              :data-testid="`skill-select-${s.id}`"
      ...
      <section class="card editor-panel">
        <h2>Editor</h2>
        <SkillEditForm v-if="selected" :skill="selected" />
        <p v-else class="muted">Select a skill to edit.</p>
      </section>

      <section class="card actions-panel">
        <h2>Validate &amp; publish</h2>

        <div class="pending-diff" data-testid="skill-pending-diff">
        ...
        <button data-testid="skill-validate-button" :disabled="validating" @click="runValidate">
          {{ validating ? 'Validating…' : 'Validate' }}
        </button>

        <p v-if="validation?.valid" class="ok" data-testid="skill-validate-ok">Valid — safe to publish.</p>
        <ul v-else-if="validation && !validation.valid" class="error" data-testid="skill-validate-errors">
          <li v-for="err in validation.errors" :key="err">{{ err }}</li>
        </ul>

        <label>
          Publish notes
          <textarea v-model="publishNotes" rows="2" data-testid="skill-publish-notes" />
        </label>
        <button class="primary" data-testid="skill-publish-button" :disabled="publishing" @click="runPublish">
          {{ publishing ? 'Publishing…' : 'Publish new version' }}
        </button>

        <p v-if="publishMessage" class="ok" data-testid="skill-publish-message">{{ publishMessage }}</p>
        <ul v-if="publishProblems" class="error" data-testid="skill-publish-problems">
          <li v-for="p in publishProblems" :key="p">{{ p }}</li>
        </ul>

        <h3>Roll back to</h3>
        <ul class="version-list" data-testid="skill-rollback-list">
          <li v-for="v in rollbackableVersions" :key="v">
            <span class="version-tag">{{ v }}</span>
            <button :disabled="rollingBackVersion === v" :data-testid="`skill-rollback-${v}`" @click="runRollback(v)">
        ...
        <ul v-else class="version-list" data-testid="skill-version-list">
